...
**Rebuild in progress — Phase 13 / 13-2 fully closed (security sweep + POPIA compliance: 13-2a RLS audit + atomic RPC + CSP-Report-Only; 13-2b end-to-end deletion lifecycle via soft-delete + 30-day grace + pg_cron anonymise + Vercel Cron auth-ban + `/api/me/export` data portability + cross-user "Deleted player" rendering). Phase 13 / 13-3 (performance + Lighthouse + real-device confirmation + rounded-xl tier sweep) next.** This repository is being rebuilt from a fresh Supabase project, fresh routing, a fresh design system, and a three-role architecture. See [`HANDIBOWLS_REBUILD_PLAN.md`](HANDIBOWLS_REBUILD_PLAN.md) for the phase-gated plan and [`PHASE_LOG.md`](PHASE_LOG.md) for the canonical progress tracker.
