...
**Rebuild in progress — Phase 13 / 13-7 fully closed (launch infrastructure: nonce-based CSP enforcement via `proxy.ts` middleware + new `lib/security/csp.ts` builder — per-request nonces stamped on every response shape, no `'unsafe-inline'` on scripts, `vercel.live` conditionally allow-listed for non-production deploys, `'unsafe-eval'` conditionally for local dev HMR; CSP curl-verified per-route on /privacy /terms /help /help/[slug] / /login plus operator-confirmed zero inline-script violations on Vercel preview DevTools console; Resend graceful-failure mode hardened — `sendEmail` returns discriminated `{ok,kind,error}` results with new `domain_not_verified` kind for the v1 onboarding-sender baseline + structured `[email:skipped]` console.warn for `vercel logs` triage; React 19 RSC `Connection closed.` benign bfcache noise suppressed at the Sentry SDK level via `beforeSend` filter; cron auth contract verified at `/api/cron/anonymise-pending` (existing integration coverage at `tests/integration/actions/cron-anonymise-pending.test.ts`); public-route allowlist hotfix added /privacy + /terms + /help + /help/[slug] to `isPublicPath` (latent defect from 13-6 Batches A+B that surfaced when the proxy first ran auth-routing on those routes); PII scrub on public marketing surfaces (`Andrew Els` → `HandiBowls`; `a.thomas.els@gmail.com` → `support@handibowls.co.za`); operator-facing `LAUNCH_DEPLOY_DRY_RUN.md` + `LAUNCH_DNS_CHECKLIST.md` banking the post-DNS launch sequence — env-var inventory, smoke-test checklist, Sentry verification via SENTRY_TEST=throw env-gated pattern, cron verification, 3 rollback procedures, Vercel custom domain + Resend domain verification + DMARC + Better Stack swap + RESEND_FROM swap + 10 common failure modes; DNS-dependent work — custom domain cutover, Resend domain verification, all 6 Supabase email-template branding, `support@handibowls.co.za` forwarder registration — banked for post-DNS-access launch). Phase 13 / 13-8 (pre-launch QA + 13-3 real-device perf gate + final operator decision on DNS-vs-preview launch posture) next.** This repository is being rebuilt from a fresh Supabase project, fresh routing, a fresh design system, and a three-role architecture. See [`HANDIBOWLS_REBUILD_PLAN.md`](HANDIBOWLS_REBUILD_PLAN.md) for the phase-gated plan and [`PHASE_LOG.md`](PHASE_LOG.md) for the canonical progress tracker.
...
Pending: Phase 13 / 13-8 (pre-launch QA + 13-3 real-device perf gate + final operator decision on DNS-vs-preview launch posture). Open drift items tracked in [`DRIFT_LOG.md`](DRIFT_LOG.md).
