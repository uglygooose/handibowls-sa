...
**Rebuild in progress — Phase 13 / 13-5 fully closed (observability: Sentry browser/server/edge SDK with POPIA-conscious defaults — `sendDefaultPii: false`, no Replay, id-only `setUser` — wired into auth listener + 3 audit_log RPC action sites via `captureWithAuditContext` helper; CSP `connect-src` extended for Sentry ingest + `report-uri`/`report-to` directives wire CSP violations into the same Sentry project; source-map upload enabled day one for symbolicated production stack traces; public `/api/health` endpoint with service-role probe + 200/503 contract; Better Stack uptime monitors configured for landing + health at 3-min cadence with email-only alerts; Supabase log drain deferred post-launch; CSP enforcement flip from Report-Only banked for 13-7). Phase 13 / 13-6 (onboarding checklists + /help seed articles + legal/privacy copy) next.** This repository is being rebuilt from a fresh Supabase project, fresh routing, a fresh design system, and a three-role architecture. See [`HANDIBOWLS_REBUILD_PLAN.md`](HANDIBOWLS_REBUILD_PLAN.md) for the phase-gated plan and [`PHASE_LOG.md`](PHASE_LOG.md) for the canonical progress tracker.
...
Pending: Phase 13 / 13-6 (content) → 13-7 (launch infra + CSP enforcement flip + DNS) → 13-8 (pre-launch QA + 13-3 real-device perf gate). Open drift items tracked in [`DRIFT_LOG.md`](DRIFT_LOG.md).
