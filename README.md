...
**Rebuild in progress — Phase 13 / 13-6 fully closed (content + copy polish: bespoke POPIA-aligned `/privacy` and governing-law-RSA `/terms` public pages under `app/(public)/`, footer + signup-form agree-checkbox + `/me/settings/data-and-privacy` header all wired; `/help` route at `app/(public)/help/` with 4 SSG seed articles — creating a tournament, scoring a match, booking a rink, Twenty 20 walkthrough — pre-draft survey caught 4 brief-vs-source divergences (Platinum band, "20 ends across 4 sections", non-existent 'closed' tournament status, "one-way" rubric activation) corrected before drafting; Footer Clubs column "Docs" → "Help" rewire; `AdminGettingStartedChecklist` data-driven 5-item card on `/manage/overview` with collapsed-strip all-complete state — checked state derived from real schema queries via `getOnboardingChecklistState(clubId)`, no localStorage, no manual dismissal; BSA terminology mechanical sweep — 7 user-visible swaps across 6 files (`T20` → "Twenty 20" ×2, `lawn bowls` → "bowls" ×4, `handicap points` → "handicap shots" ×1) with documented skips for /t20 Platinum tier (honours open DRIFT entry — aspirational design fidelity) and `AssessmentResults` rubric points (T20 schema-defined, not bowls match shots)). Phase 13 / 13-7 (launch infra: DNS pointing for `app.handibowls.co.za` + CSP enforcement flip from Report-Only + Resend DMARC/DKIM/SPF + Vercel `CRON_SECRET` verification) next.** This repository is being rebuilt from a fresh Supabase project, fresh routing, a fresh design system, and a three-role architecture. See [`HANDIBOWLS_REBUILD_PLAN.md`](HANDIBOWLS_REBUILD_PLAN.md) for the phase-gated plan and [`PHASE_LOG.md`](PHASE_LOG.md) for the canonical progress tracker.
...
Pending: Phase 13 / 13-7 (launch infra + CSP enforcement flip + DNS) → 13-8 (pre-launch QA + 13-3 real-device perf gate). Open drift items tracked in [`DRIFT_LOG.md`](DRIFT_LOG.md).
