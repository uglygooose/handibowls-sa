...
**Rebuild in progress — Phase 13 / 13-2a closed (security sweep — RLS audit across 4 named protected mutations + atomic activate_rubric_version RPC via migration 042 + Content-Security-Policy-Report-Only headers ship for production observation; 0 violations on Vercel preview production build). Phase 13 / 13-2b (POPIA compliance — soft-delete schema, /api/me/export, account deletion flow, retention policy) next.** This repository is being rebuilt from a fresh Supabase project, fresh routing, a fresh design system, and a three-role architecture. See [`HANDIBOWLS_REBUILD_PLAN.md`](HANDIBOWLS_REBUILD_PLAN.md) for the phase-gated plan and [`PHASE_LOG.md`](PHASE_LOG.md) for the canonical progress tracker.
