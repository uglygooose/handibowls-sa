...
**Rebuild in progress — Phase 12.5 closed (design fidelity & unification — primitives + bundle alignment across all admin / super-admin / player surfaces). Phase 13 (technical polish + go-live) next.** This repository is being rebuilt from a fresh Supabase project, fresh routing, a fresh design system, and a three-role architecture. See [`HANDIBOWLS_REBUILD_PLAN.md`](HANDIBOWLS_REBUILD_PLAN.md) for the phase-gated plan and [`PHASE_LOG.md`](PHASE_LOG.md) for the canonical progress tracker.
...
Pending: technical polish (Phase 13). Open drift items tracked in [`DRIFT_LOG.md`](DRIFT_LOG.md).
