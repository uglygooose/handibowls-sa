...
**Rebuild in progress — Phase 13 / 13-1 closed (a11y audit + WCAG 2.1 AA fixes — Stage 2 close-gate met across 10 anchor surfaces; 9/10 at Lighthouse a11y 100 with 1 surface at 96 deferred to 13-3 as a token-level decision). Phase 13 / 13-2 (security sweeps) next.** This repository is being rebuilt from a fresh Supabase project, fresh routing, a fresh design system, and a three-role architecture. See [`HANDIBOWLS_REBUILD_PLAN.md`](HANDIBOWLS_REBUILD_PLAN.md) for the phase-gated plan and [`PHASE_LOG.md`](PHASE_LOG.md) for the canonical progress tracker.
