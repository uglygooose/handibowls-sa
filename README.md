...
**Rebuild in progress — Phase 13 / 13-3 fully closed (perf measurement banked for 13-8 real-device + visual residuals + a11y class-of-bug + rounded-xl tier sweep across 88 files). Phase 13 / 13-4 (SEO + landing brand story + favicon + OG + sitemap + robots) next.** This repository is being rebuilt from a fresh Supabase project, fresh routing, a fresh design system, and a three-role architecture. See [`HANDIBOWLS_REBUILD_PLAN.md`](HANDIBOWLS_REBUILD_PLAN.md) for the phase-gated plan and [`PHASE_LOG.md`](PHASE_LOG.md) for the canonical progress tracker.
...
Pending: Phase 13 / 13-4 (SEO) → 13-5 (monitoring) → 13-6 (content) → 13-7 (launch infra) → 13-8 (pre-launch QA + 13-3 real-device perf gate). Open drift items tracked in [`DRIFT_LOG.md`](DRIFT_LOG.md).
