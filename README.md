...
**Rebuild in progress — Phase 12 closed (stakeholder polish — player /t20 hub + booking loop, tournament admin gaps, messaging + notification polish, T20 admin polish, performance sweep, design fidelity touch-ups, pre-stakeholder QA + cross-cutting fixes). Phase 12.5 (Claude Design pass) next.** This repository is being rebuilt from a fresh Supabase project, fresh routing, a fresh design system, and a three-role architecture. See [`HANDIBOWLS_REBUILD_PLAN.md`](HANDIBOWLS_REBUILD_PLAN.md) for the phase-gated plan and [`PHASE_LOG.md`](PHASE_LOG.md) for the canonical progress tracker.
...
Pending: technical polish (Phase 13) · Claude Design pass (Phase 12.5). Open drift items tracked in [`DRIFT_LOG.md`](DRIFT_LOG.md).
