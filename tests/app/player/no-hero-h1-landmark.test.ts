...
// landmark on the four no-hero player surfaces. Bundle prescribes
// no visible h1 on /play / /book / /tournaments / /me/inbox
// (player-pages.jsx PagePlay / PageBook / PageInbox + player-
// core.jsx PageTournaments — all start with .mcontent + tab-bar /
// notif-banner / etc, no page-level h1). Shipped surfaces wrap
// the existing h1 in `<header className="sr-only">` (or apply
// sr-only directly on the h1) so screen readers still get a
// page landmark, but visually nothing renders.
//
// Static-grep style assertions — the four surfaces are Server
// Components with auth + data fetches that don't render cleanly
// in Vitest. Reading the source as text is the pragmatic guard.

const SURFACES = [
  "app/(player)/(gated)/play/page.tsx",
