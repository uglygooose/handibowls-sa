...
- [ ] **TanStack Table + React Compiler lint warning.** `react-hooks/incompatible-library` warning on `useReactTable()` — library returns functions the compiler can't memoise. Upstream TanStack issue; warning is not an error and is unsuppressable without disabling the rule. Revisit when TanStack ships a compiler-friendly API or an ESLint-rule escape hatch. File: `app/(super-admin)/platform/clubs/_components/ClubsTable.tsx`. Discovered: Phase 4b, 2026-04-23.
